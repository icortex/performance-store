                ...
                if (e.is(":visible") && (typeof b.copy == "string" || a.isFunction(b.copy))) {
                    ...
                    d.addEventListener("complete", function(f, g) {
                        if (a.isFunction(b.afterCopy)) {
                            ...
                            e.removeClass("hover");
                            alert("Los emails han sido copiados al portapapeles")
                        }
                        if (b.clickAfter) {
                            e.trigger("click")
                        }
...
var ZeroClipboard = {version:"1.0.7",clients:{},moviePath:"ZeroClipboard.swf",nextId:1,jq:function(a) {
    if (typeof(a) == "string") {
...
ZeroClipboard.Client.prototype = {id:0,ready:false,movie:null,clipText:"",handCursorEnabled:true,cssEffects:true,handlers:null,glue:function(d, b, e) {
    this.domElement = ZeroClipboard.jq(d);
    var f = 99;
    if (this.domElement.style.zIndex) {
        f = parseInt(this.domElement.style.zIndex, 10) + 1
    }
    if (typeof(b) == "string") {
        b = ZeroClipboard.jq(b)
    } else {
    ...
    jq(this.domElement).data("zclipId", "zclip-" + this.movieId);
    ...
    if (c) {
        this.domElement = ZeroClipboard.jq(c);
        if (!this.domElement) {
            this.hide()
        }
    }
    ...
    switch (d) {case"load":this.movie = document.getElementById(this.movieId);if (!this.movie) {
        var c = this;
        setTimeout(function() {
    ...
    }break;case"mouseover":if (this.domElement && this.cssEffects) {
    ...
    }break;case"mousedown":if (this.domElement && this.cssEffects) {
        this.domElement.addClass("active")
    }break;case"mouseup":if (this.domElement && this.cssEffects) {
        this.domElement.removeClass("active");
        this.recoverActive = false
    }break
    }
